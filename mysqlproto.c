...
#ifdef DEBUG
#define PRINT_CAPABILITY(NAME)\
	if (capabilities & NAME) \
		printf(" " #NAME);
	printf("capabilities:");
	PRINT_CAPABILITY(CLIENT_LONG_PASSWORD);
	PRINT_CAPABILITY(CLIENT_FOUND_ROWS);
	PRINT_CAPABILITY(CLIENT_LONG_FLAG);
	PRINT_CAPABILITY(CLIENT_CONNECT_WITH_DB);
	PRINT_CAPABILITY(CLIENT_NO_SCHEMA);
	PRINT_CAPABILITY(CLIENT_COMPRESS);
	PRINT_CAPABILITY(CLIENT_ODBC);
	PRINT_CAPABILITY(CLIENT_LOCAL_FILES);
	PRINT_CAPABILITY(CLIENT_IGNORE_SPACE);
	PRINT_CAPABILITY(CLIENT_PROTOCOL_41);
	PRINT_CAPABILITY(CLIENT_INTERACTIVE);
	PRINT_CAPABILITY(CLIENT_SSL);
	PRINT_CAPABILITY(CLIENT_IGNORE_SIGPIPE);
	PRINT_CAPABILITY(CLIENT_TRANSACTIONS);
	PRINT_CAPABILITY(CLIENT_RESERVED);
	PRINT_CAPABILITY(CLIENT_SECURE_CONNECTION);
	PRINT_CAPABILITY(CLIENT_MULTI_STATEMENTS);
	PRINT_CAPABILITY(CLIENT_MULTI_RESULTS);
	PRINT_CAPABILITY(CLIENT_PS_MULTI_RESULTS);
	PRINT_CAPABILITY(CLIENT_PLUGIN_AUTH);
	PRINT_CAPABILITY(CLIENT_CONNECT_ATTRS);
	PRINT_CAPABILITY(CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA);
	PRINT_CAPABILITY(CLIENT_CAN_HANDLE_EXPIRED_PASSWORDS);
	PRINT_CAPABILITY(CLIENT_SESSION_TRACK);
	PRINT_CAPABILITY(CLIENT_DEPRECATE_EOF);
	PRINT_CAPABILITY(CLIENT_SSL_VERIFY_SERVER_CERT);
	PRINT_CAPABILITY(CLIENT_REMEMBER_OPTIONS);
	printf("\n");
#endif
